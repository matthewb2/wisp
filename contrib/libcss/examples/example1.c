...
int main(int argc, char **argv)
{
    ...
    for (hh = 1; hh != 7; hh++) {
        ...
        code = css_select_style(select_ctx, element_name, &unit_len_ctx, &media, NULL, &select_handler, 0, &style);
        if (code != CSS_OK)
...
css_error node_has_name(void *pw, void *n, const css_qname *qname, bool *match)
{
    lwc_string *node = n;
    UNUSED(pw);
    assert(lwc_string_caseless_isequal(node, qname->name, match) == lwc_error_ok);
    return CSS_OK;
}
...
css_error node_has_attribute_equal(void *pw, void *n, const css_qname *qname, lwc_string *value, bool *match)
{
    UNUSED(pw);
    UNUSED(n);
    UNUSED(qname);
